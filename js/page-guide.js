...
RR.pageGuide = (function () {
  ...
  function control(routeId) {
    ...
    // Speaker — read the explanation aloud (only where speech is supported). One
    // job plays at a time; the engine resets this button via onEnd when superseded.
    if (RR.tts && RR.tts.supported()) {
      ...
      listen.addEventListener("click", function () {
        if (listen.classList.contains("is-playing")) { RR.tts.cancel(); return; }
        ...
        RR.tts.speak(speechFor(routeId), {
